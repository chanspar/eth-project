...
from utils.spark_spec_gke import build_spark_spec, SPARK_NAMESPACE, K8S_CONN_ID

# ── Asset 정의 ────────────────────────────────────────────────────────────────
BRONZE_GKE_COMPLETE = Asset("bronze/ethereum_etl_gke_complete")
SILVER_GKE_COMPLETE = Asset("silver/ethereum_silver_gke_complete")
...
def ethereum_silver_gke_dag():
    ...
    def get_execution_date() -> str:
        ...
        events = (context.get("triggering_asset_events") or {}).get(BRONZE_GKE_COMPLETE, [])
        if events:
    ...
    @task(outlets=[SILVER_GKE_COMPLETE])
    def publish_silver_asset(dt_str: str):
        ...
        yield Metadata(SILVER_GKE_COMPLETE, {"logical_date": dt_str})
